...
## Marco 4 — Offline-First + Integração e-SUS 📡 ✅ CONCLUÍDO
> **Objetivo:** O sistema funciona 100% sem internet. Sincroniza com e-SUS quando possível.

### Entregáveis

#### Offline-First
- [x] Service Worker cacheando o shell do frontend (PWA)
- [x] Frontend detecta status online/offline e exibe indicador visual
- [x] Todas as operações críticas (gerar senha, chamar fila) funcionam sem rede
- [x] UUIDs como IDs primários em todas as entidades

#### Motor de Sincronização
- [x] `SyncService` no backend — tenta enviar dados para o e-SUS periodicamente
- [x] Fila de retry com backoff exponencial (1s → 2s → 4s → ... → 5min max)
- [x] Log local de eventos pendentes de sincronização
- [x] Endpoint `GET /sync/status` — retorna quantos registros estão pendentes

#### Integração e-SUS PEC (LEDI APS)
- [x] `EsusGateway` real — envia fichas via API REST (HTTPS) (Via Mock no momento)
- [x] Autenticação com credenciais geradas no PEC
- [x] Fallback para `MockEsusGateway` quando em modo desenvolvimento
